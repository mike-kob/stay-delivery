...
export const adminSignupWithEmail = async (email, password) => {
  console.log('INNER');
  const userCredential = await auth.createUserWithEmailAndPassword(email,
      password);
  console.log(userCredential);
  ...
  const res = await axios.post('/auth/signup/restaurant',
      {id_token: token, name: 'name', description: 'description'},
      {withCredentials: true},
  );
  console.log(res, 'POSTED');
};
